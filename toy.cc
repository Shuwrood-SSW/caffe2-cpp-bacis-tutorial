...
void run()
{
  ...
  Workspace workspace; //创建workspace

  // >>> init_net = core.Net("init")
  NetDef initModel; // 创建 initModel
  ...
  // 初始化 ITER
  {
    ...
    op->set_type("ConstantFill");
    auto arg1 = op->add_arg();
    arg1->set_name("shape")  // >>> print("Before training, W is: {}".format(workspace.FetchBlob("W")))
;
    arg1->add_ints(1);
    auto arg2 = op->add_arg();
    arg2->set_name("value");
  ...
  //构建训练模型（包括训练样本初始化，训练网络搭建，设置损失函数）

  // >>> train_net = core.Net("train")
  NetDef trainModel;
  trainModel.set_name("train");

  // >>> X = train_net.GaussianFill([], "X", shape=[64, 2], mean=0.0, std=1.0,  代表什么样的数据？
  // run_once=0)
  // 构建训练样本 X
  {
  ...
  // 运行初始化模型
  CAFFE_ENFORCE(workspace.RunNetOnce(initModel));//为什么不需要create?

  // >>> workspace.CreateNet(train_net)  print(workspace.GetBlob("B"), "B before");
  // 创建训练模型
  ...
  //打印网络
  // std::cout << workspace.LocalBlobs() << std::endl;
  // std::cout << workspace.RootFolder() << std::endl;
  // std::cout << trainModel.DebugString() << std::endl;
  // std::cout << trainModel.name() << std::endl;
